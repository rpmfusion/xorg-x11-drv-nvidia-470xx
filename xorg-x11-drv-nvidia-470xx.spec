%global        _nvidia_serie        nvidia-470xx
...
Requires:         which
Requires:         nvidia-settings-470xx%{?_isa} = %{?epoch}:%{version}
%if 0%{?fedora} || 0%{?rhel} > 7
...
- GeForce 1000(M) series
- GeForce 1600(M) series
- GeForce 2000(M) series
...
tar Jcf %{buildroot}%{_datadir}/%{_nvidia_serie}-kmod-%{version}/%{_nvidia_serie}-kmod-%{version}-x86_64.tar.xz kernel
...
%{SOURCE9} README.txt | xargs appstream-util add-provide %{buildroot}%{_metainfodir}/xorg-x11-drv-%{_nvidia_serie}.metainfo.xml modalias
...
%{_datadir}/%{_nvidia_serie}-kmod-%{version}/%{_nvidia_serie}-kmod-%{version}-x86_64.tar.xz
%endif
...
%changelog
* Sun Dec 21 2025 Sérgio Basto <sergio@serjux.com> - 3:470.256.02-5
- Uodate the package version in the trigger condition, otherwise it will not run
...
* Tue Apr 05 2022 Leigh Scott <leigh123linux@gmail.com> - 3:470.103.01-4
- Fix appdata

* Sat Feb 12 2022 Sérgio Basto <sergio@serjux.com> - 3:470.103.01-3
